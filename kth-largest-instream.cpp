...
// Approach 1 map & counts, buckets
// only sort k unique elements, 
// touch/modify the keys less if there are a lot of duplicate elements in nums
class KthLargest {
  public:    
    KthLargest(int k, vector<int>& nums) {
      this->k = k; c = 0; n_counts.clear(); 
      ...
      for (vector<int>::iterator itr = nums.begin(); itr != nums.end(); ++itr) {
        ...
        if (c < k) {  //have not seen at least k elements yet
          ...
          if (mitr != n_counts.end())
            mitr->second += 1;
          else { //add n to map, trigger resort of n_counts only if n is new
...
// Approach 2 multiset, like a priority queue
// need to keep sorting all k elements
class KthLargest_2 {
  public:    
    KthLargest_2(int k, vector<int>& nums) {
      this->k = k; knums.clear();
      int sz = nums.size();
      if (0 == sz) return;
      else if (sz < k)
        knums.insert(nums.begin(), nums.end());
      else
        knums.insert(nums.begin(), nums.begin() + k);
      for (int i = k; i < sz; ++i)
        add(nums.at(i));
    }
    ...
    int add(int val) { 
      knums.insert(val);
      if (knums.size() > k)
        knums.erase(knums.begin()); //remove the smallest element
      return *knums.begin();  //the first element
    }
...
int main() {
  ...
  KthLargest_2 *obj = new KthLargest_2(k, nums_v);
  cout << obj->add(-3) << endl;  //-3
  cout << obj->add(-2) << endl;  //-2
  cout << obj->add(-4) << endl;  //-2
  cout << obj->add(0) << endl;    //0
